...
@section('content')
    ...
    <!-- 01 Header Form-->
                <div class="modal-header">
                    ...
                    <h3 class="smaller lighter blue no-margin">Form Data Cuti</h3>
                </div>
    <!-- 01 end heder form-->
    <!-- 02 body Form -->
                <div class="modal-body">
                    <form class="form-horizontal form-aktif" id="form-1" method="get">
                      {!! csrf_field() !!}
                      <input type="hidden" name="datatb" value="dct" />
                      ...
                      <div class="row">
                        <label class="control-label col-sm-3 no-padding-right" for="name">Cuti Terhitung :</label>
                        <div class="col-sm-5">
                          <input id="jcuti" name="jcuti" type="checkbox" value="1" class="ace ace-switch ace-switch-4" checked/>
                          <span class="lbl" data-lbl="&nbsp;O&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;III"></span>
                          <small class="green">
                              <b>Tidak Terhitung / Terhitung</b>
                          </small>
                        </div>
                      </div><div class="space-2"></div>

                      <div class="row">
                          <label class="control-label col-sm-3 no-padding-right" for="name">Jatah Cuti :</label>
                          <div class="col-sm-4">
                            <input type="text" id="jthcuti" name="jthcuti" value="" disabled class="col-sm-5"/>
                            <span class="help-inline col-sm-7">
                              <label class="middle">
                                  <input id="id-disable-check" name="ujc" class="ace" type="checkbox">
                      ...
                      <div class="row">
                          ...
                          <label class="control-label col-sm-1 no-padding-right" id="sisa">0</label>
                          <label class="control-label col-sm-3 no-padding-right" id="pakai"> - Digunakan 0 Hari</label>
                      </div><div class="space-2"></div>
    ...
    <!-- end Form -->
            <div align="center">Data Cuti<br />PT. PELABUHAN CILEGON MANDIRI<br />
    ...
    jQuery(function($) {
      ...
      getparameter(site+'/api/kepegawaian/json',posdata,function(data){
        ...
        jQuery(grid_selector).jqGrid({
            ...
            sortname:'no',
            sortorder: 'desc',
            postData: {datatb:'dct',_token:'{{ csrf_token() }}'},
            height: 250,
            colNames:['No','Nama','Jenis Cuti', 'Mulai','Akhir','Cuti','Sisa','Ket'],
            colModel:[
                {name:'no',index:'no', width:50, editable:false},
                {name:'userid',index:'userid',width:120, editable:false },
                {name:'jenis',index:'jenis', width:90, editable: false},
                ...
                {name:'lama',index:'lama', width:50, editable: false},
                {name:'sisa',index:'sisa', width:100, editable: false},
            ...
            */
            ...
            onClickButton:function(){
                ...
                if(gsr){
                    ...
                    var postData= {datatb:'nodatacuti', cari: gsr, _token:'<?php echo csrf_token();?>', oper:'pra-edit'};
                    $.ajax({
                        ...
                        success: function(response){
                            ...
                            if(response.jcuti==1)$('#jcuti').prop('checked',true);else $('#jcuti').prop('checked',false);

                            $('#jthcuti').get(0).setAttribute('disabled' , 'disabled');
                            $('#jthcuti').val(response.jthcuti);


                            $(".tgl").prop('disabled', false);

                            $.post( "{{url('/api/kepegawaian/json')}}", {
                                datatb:'haricuti',
                                _token:'{{ csrf_token() }}',
                                sdate:response.sdate,
                                edate:response.edate,
                                cari:response.id_u,
                                wkerja:response.wkerja,
            ...
            onClickButton:function(){
                ...
                var postData= {datatb:'nodatacuti', _token:'{{ csrf_token() }}', oper:'pra-add'};
                $.ajax({
                    url: "{{url('/api/kepegawaian/json')}}",
                    data : postData,
                    type: "post",
        ...
        $( "#ket" ).autocomplete({
            source: function( request, response ) {
                $.ajax({
                    ...
                    type: "post",
                    dataType: "json",
                    data: {datatb:'uraiancuti', cari: request.term, _token:'{{ csrf_token() }}', oper:'edit'},
